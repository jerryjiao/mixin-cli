...
const action = async () => {
  ...
  if (fs.existsSync(folder)) {
    console.log(`${cross} Folder ${chalk.cyan(folder)} already exist! Remove the directory or choose another name!`);
    process.exit(0);
    return;
  }
  ...
  if (args.type === TYPE_REACT) {
    ...
    const child = spawn(
      'create-react-app',
      `${args.name} --scripts-version mixin-react-scripts`.split(/\s+/),
      spawnOptions
    );

    child.on('close', () => {
      console.log('');
      console.log(`${check} DApp initialized successfully!`);
      console.log('');
      process.exit(0);
    });
  ...
  if (args.type === TYPE_VUE) {
    ...
    const child = spawn('vue', `create --preset wangshijun/mixin-vue-starter ${args.name}`.split(/\s+/), spawnOptions);
...
module.exports = program => {
  program
    .command('dapp:create')
    .description('Bootstrap a new DAPP that have mixin-client-sdk integrated')
    .action(action);
};
