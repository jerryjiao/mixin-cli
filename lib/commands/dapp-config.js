...
const action = async () => {
  ...
  const { pin: assetPin, session_id: sessionId, pin_token: encryptedAESKey, private_key: privateKeyStr } = JSON.parse(
    sessionInfo
  );
  if (!assetPin || !sessionId || !encryptedAESKey || !privateKeyStr.length) {
  ...
  try {
    ...
    console.log(
      `${cross} sessionInfo decode failed with error: ${error.message || error.toString()}, submit an issue at ${
        bugs.url
      }`
    );
    return process.exit(1);
...
MIXIN_PRIVATE_KEY="${privateKeyStr.replace(/\n/g, '\\n').replace(/\r/g, '\\r')}"`
    );
    console.log(`${check} Config file written to ${filePath}`);
    console.log(`${check} Press ctrl+v and then enter to view file content`);
    clipboard.writeSync(`cat ${filePath}`);
